...
app.post("/sign-up", function (req, res, next) {
    if (checkKeys(req.body, ["name", "firstName", "lastName", "password"])) {
        ...
        database.add(newUser, function (result) { res.send(result); });
    } else {
        res.send("false");
    }
});
    ...
    if (checkKeys(req.body, ["name"])) {
        ...
        database.lookup(user, function (err, result) {
            ...
            if (result.name === user.name) {
                ...
                newUser.step();
                database.update(user, newUser, function (result) { res.send(result); });
            } else {
                res.send("false");
            }
        });
    ...
    if (checkKeys(req.body, ["name", "height", "weight", "strideLength"])) {
        ...
        database.lookup(user, function (err, result) {
            ...
            if (user.name === result.name) {
                ...
                newUser.strideLength = req.body.strideLength;
                database.update(user, newUser, function (result) { res.send(result); });
            } else {
                res.send("false");
            }
...
app.post("/set-goal", function (req, res, next) {
    if (checkKeys(req.body, ["name", "goalType", "goalValue"])) {
        ...
        database.lookup(user, function (err, result) {
            ...
            if (user.name === result.name) {
                newUser = (new User()).genUserFromObject(result);
                newUser.goal = new Goal().createNewGoal(req.body.goalType, req.body.goalValue);
                database.update(user, newUser, function (result) { res.send(result); });
            } else {
                res.send("false");
            }
